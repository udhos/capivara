...
var pieceSquareBishop = [64]int16{
...
var pieceSquareKnight = [64]int16{
...
var pieceSquareTable = [6][64]int16{
	pieceSquareKing,   // king
	pieceSquareQueen,  // queen
	pieceSquareRook,   // rook
	pieceSquareBishop, // bishop
	pieceSquareKnight, // knight
	pieceSquarePawn,   // pawn
}

// mirrorPieceSquareTable swaps these rows:
// 1 <=> 8
// 2 <=> 7
// 3 <=> 6
// 4 <=> 5
// why? because the literal piece square tables above are in inverse rank order.
// they are in inverse order because that order is visually easier on the eye.
func mirrorPieceSquareTable() {
	log.Printf("mirrorPieceSquareTable: mirroring 4 first rows x 4 last rows")
	for k := 0; k < 6; k++ {
		for row := 0; row < 4; row++ {
			for col := 0; col < 7; col++ {
				loc1 := row*8 + col
				loc2 := (7-row)*8 + col
				pieceSquareTable[k][loc1], pieceSquareTable[k][loc2] = pieceSquareTable[k][loc2], pieceSquareTable[k][loc1] // swap
			}
		}
	}
